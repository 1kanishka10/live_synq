...
// It is not "money is mentioned" that matters — colleges bill students all the
// time. What separates a fraud from a fee is the RAIL: a scam needs the money
// to leave through the message itself, by transfer, QR or link. A real due is
// settled somewhere you already go.
const RAIL_RE =
  /\bupi\b|\bg-?pay\b|\bgoogle pay\b|\bpaytm\b|\bphonepe\b|\bqr\s*code\b|\bscan (?:the|this)\b|\bifsc\b|\baccount (?:number|no\.?)\b|\bpayment link\b|\bpay (?:here|now|via|through|using|to)\b/i;

const IN_PERSON_RE =
  /\bat the counter\b|\bin person\b|\bat the (?:office|desk|window|branch)\b|\b(?:student|hostel|college|university|exam(?:ination)?) portal\b|\baccounts (?:office|section)\b|\b(?:cash|fee|library) counter\b|\bwarden'?s? office\b/i;
...
export function assess(item, opts = {}) {
  ...
  const rail = RAIL_RE.test(body);
  const inPerson = IN_PERSON_RE.test(body);
  ...
  // Decisive only when the money is meant to move THROUGH the message — a
  // transfer, a QR, or a link. "Fines must be paid at the counter by Friday"
  // is a real obligation settled somewhere you already go, and flagging it
  // taught students to ignore the flag.
  const moneyOffRails = asksMoney && (rail || offsite.length > 0);

  if (moneyOffRails) {
    reasons.push({
      code: "payment_requested",
      headline: offsite.length
        ? "It asks you to pay through a link outside the college."
        : "It asks you to transfer money directly, from inside the message.",
      detail:
        "No government or college scholarship in India charges a fee to apply, and a genuine due — a fine, a mess bill, an exam fee — is settled at a counter or on a portal you already have a login for. Money that has to leave through a chat message is the one thing a fraud cannot do without.",
    });
  } else if (asksMoney) {
    notes.push({
      code: "payment_in_person",
      headline: inPerson
        ? "It mentions money, but payable in person or through a college portal."
        : "It mentions money, with no transfer details given.",
      detail:
        "Nothing is being collected through this message, so there is nothing here for a stranger to take. Colleges bill students constantly; that on its own is not a reason to doubt a notice.",
    });
  }
...
function buildChecks(reasons, notes, claimed, offsite) {
  ...
  if (codes.has("payment_requested")) {
    out.push(
      "Do not transfer anything. A real fee is paid at a counter or on a portal you already log into, never through a link or UPI id sent in a chat."
    );
  }
